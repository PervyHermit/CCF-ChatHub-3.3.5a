...
local hub, menu, minimapButton
local page = "lfg"
local boardOffset, trainerOffset, activityOffset = 0, 0, 0
local selectedActivities, trainerChecks = {}, {}
local tradeFilter, searchText = "ALL", ""
local boardRows, trainerRows, activityRows = {}, {}, {}
local MAX_BOARD, MAX_TRAINER, MAX_ACTIVITY = 22, 15, 18
...
local function MenuLabels()
    ...
    menu.buttons[1]:SetText("Looking for Group  (" .. #CCF.boards.lfg .. ")")
    menu.buttons[2]:SetText("Trade  (" .. #CCF.boards.trade .. ")")
    menu.buttons[3]:SetText("Spam Trainer Suggestions  (" .. #CCF.db.trainer.pending .. ")")
end
...
local function CreateMinimap()
    ...
    minimapButton:SetScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine("Left-click: menu", .8, .8, .8)
...
local function CreateMenu()
    ...
    menu:SetWidth(245); menu:SetHeight(154); menu:SetFrameStrata("DIALOG"); menu:SetClampedToScreen(true)
    ...
    menu.buttons = {}
    local names = { "Looking for Group", "Trade", "Spam Trainer Suggestions", "CCF Options" }
    local index
    for index = 1, 4 do
    ...
    menu.buttons[2]:SetScript("OnClick", function() menu:Hide(); CCF:OpenPage("trade") end)
    menu.buttons[3]:SetScript("OnClick", function() menu:Hide(); CCF:OpenPage("trainer") end)
    menu.buttons[4]:SetScript("OnClick", function() menu:Hide(); CCF:OpenOptions() end)
...
local function CreateBoardRow(parent, index)
    ...
    row.message = row:CreateFontString(nil,"OVERLAY","GameFontHighlightSmall"); row.message:SetPoint("LEFT",200,0); row.message:SetPoint("RIGHT",-122,0); row.message:SetJustifyH("LEFT"); row.message:SetWordWrap(false)
    row.age = row:CreateFontString(nil,"OVERLAY","GameFontDisableSmall"); row.age:SetPoint("RIGHT",-73,0); row.age:SetWidth(42); row.age:SetJustifyH("RIGHT")
    row.tell = Button(row,"Whisper",66,21); row.tell:SetPoint("RIGHT",-3,0)
    row.hit = CreateFrame("Button",nil,row); row.hit:SetPoint("TOPLEFT"); row.hit:SetPoint("BOTTOMRIGHT",-72,0)
...
local function CreateTrainerRow(parent, index)
    local row=CreateFrame("Frame",nil,parent); row:SetHeight(46); row:EnableMouse(true)
    if index%2==0 then local bg=row:CreateTexture(nil,"BACKGROUND"); bg:SetAllPoints(); bg:SetTexture(1,1,1,.035) end
    row.check=CreateFrame("CheckButton",nil,row,"UICheckButtonTemplate"); row.check:SetWidth(24); row.check:SetHeight(24); row.check:SetPoint("LEFT",2,0)
    row.phrase=row:CreateFontString(nil,"OVERLAY","GameFontNormalSmall"); row.phrase:SetPoint("TOPLEFT",30,-5); row.phrase:SetPoint("RIGHT",-110,0); row.phrase:SetJustifyH("LEFT")
    row.example=row:CreateFontString(nil,"OVERLAY","GameFontDisableSmall"); row.example:SetPoint("BOTTOMLEFT",30,5); row.example:SetPoint("RIGHT",-110,0); row.example:SetJustifyH("LEFT"); row.example:SetWordWrap(false)
    row.stats=row:CreateFontString(nil,"OVERLAY","GameFontHighlightSmall"); row.stats:SetPoint("RIGHT",-5,0); row.stats:SetWidth(100); row.stats:SetJustifyH("RIGHT")
    row.check:SetScript("OnClick", function(self) local e=self:GetParent().entry; if e then trainerChecks[e.id]=self:GetChecked() and true or nil end end)
    row:SetScript("OnEnter", function(self)
        if not self.entry then return end
        GameTooltip:SetOwner(self,"ANCHOR_RIGHT"); GameTooltip:AddLine('Suggested: "'..self.entry.phrase..'"',1,1,1); GameTooltip:AddLine(self.entry.example or "",.9,.9,.9,true); GameTooltip:Show()
    end)
...
local function CreateActivityRow(parent)
    local row=CreateFrame("CheckButton",nil,parent,"UICheckButtonTemplate"); row:SetHeight(24)
    row.text=row:CreateFontString(nil,"OVERLAY","GameFontHighlightSmall"); row.text:SetPoint("LEFT",24,0); row.text:SetPoint("RIGHT",-2,0); row.text:SetJustifyH("LEFT"); row.text:SetWordWrap(false)
    row:SetScript("OnClick", function(self)
        if self.activityId then selectedActivities[self.activityId]=self:GetChecked() and true or nil; boardOffset=0; CCF:RefreshHub() end
    end)
    row:SetScript("OnEnter", function(self)
        local a=self.activityId and CCF.activityById[self.activityId]; if not a then return end
...
local function RefreshActivities()
    ...
    for i=1,MAX_ACTIVITY do
        ...
        if item then row.activityId=item.id; row.text:SetText(ActivityLabel(item.activity,true).."  ("..item.count..")"); row:SetChecked(selectedActivities[item.id] and true or false); row:Show()
...
local function RefreshTrainer()
    ...
    hub.title:SetText("Spam Trainer Suggestions"); hub.subtitle:SetText("Check what should be filtered, then add checked and dismiss everything else.")
    local i
    for i=1,MAX_TRAINER do
        local row,e=trainerRows[i],nil; if i<=shownTrainer then e=pending[i+trainerOffset] end; row.entry=e
        if e then row.phrase:SetText('#'..e.id..'  "'..(e.phrase or "")..'"'); row.example:SetText("Example: "..(e.example or "")); row.stats:SetText("x"..(e.count or 0).." · "..(e.authorCount or 0).." sender"..((e.authorCount or 0)==1 and "" or "s")); row.check:SetChecked(trainerChecks[e.id] and true or false); row:Show()
        else row:Hide() end
    end
...
local function Layout()
    ...
    shownTrainer=math.max(5,math.min(MAX_TRAINER,math.floor((height-195)/47)))
    ...
    for i=1,MAX_TRAINER do trainerRows[i]:ClearAllPoints(); trainerRows[i]:SetPoint("TOPLEFT",hub.trainerContainer,"TOPLEFT",0,-((i-1)*47)); trainerRows[i]:SetPoint("RIGHT",hub.trainerContainer,"RIGHT",0,0) end
...
local function CreateHub()
    ...
    hub.finishTrainer=Button(hub,"Add Checked & Dismiss Rest",210,24); hub.finishTrainer:SetPoint("BOTTOMRIGHT",-25,26)
    hub.selectAll=Button(hub,"Check All",85,22); hub.selectAll:SetPoint("RIGHT",hub.finishTrainer,"LEFT",-8,0)
    hub.selectNone=Button(hub,"Check None",85,22); hub.selectNone:SetPoint("RIGHT",hub.selectAll,"LEFT",-5,0)
    ...
    hub.selectAll:SetScript("OnClick",function() local n for n=1,#CCF.db.trainer.pending do trainerChecks[CCF.db.trainer.pending[n].id]=true end; RefreshTrainer() end)
    hub.selectNone:SetScript("OnClick",function() trainerChecks={}; RefreshTrainer() end)
    hub.finishTrainer:SetScript("OnClick",function()
        local added,dismissed=CCF:FinalizeSuggestions(trainerChecks); trainerChecks={}; trainerOffset=0
        CCF:Print(added.." checked suggestion(s) added; "..dismissed.." unchecked suggestion(s) dismissed."); RefreshTrainer()
    end)
    hub.finishTrainer:SetScript("OnEnter",function(self) GameTooltip:SetOwner(self,"ANCHOR_TOP"); GameTooltip:AddLine("Add Checked & Dismiss Rest",1,1,1); GameTooltip:AddLine("Checked phrases are added. Unchecked patterns are dismissed and will not be suggested again.",.8,.8,.8,true); GameTooltip:Show() end)
...
local function Init()
    CreateHub(); CreateMenu(); CreateMinimap()
    CCF:RegisterCallback("BOARD_UPDATED",function(category) if hub and hub:IsShown() and page==category then CCF:RefreshHub() end; MenuLabels() end)
    ...
    CCF:RegisterCallback("OPTIONS_UPDATED",function() CCF:UpdateMinimapVisibility(); CCF:RefreshHub() end)
    local ticker=CreateFrame("Frame"); ticker.elapsed=0
    ticker:SetScript("OnUpdate",function(self,elapsed) self.elapsed=self.elapsed+elapsed; if self.elapsed<5 then return end; self.elapsed=0; if hub and hub:IsShown() and page~="trainer" then CCF:RefreshHub() end end)
end
